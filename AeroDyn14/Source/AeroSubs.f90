...
DO WHILE ( ABS( DELAI ) > ATOLERBY10 .AND. ABS(DAI) > ATOLER2 )
 ...
 !CH--  Put floor under ASTEP to keep it reasonable after many zero crossings
   ...
   IF( NINT( SIGN(1._ReKi, DAI) ) /= NINT( SIGN(1._ReKi, DAI1) ) ) ASTEP = MAX( 1.0E-4, 0.5*ASTEP )
...
IF ( ABS( CH ) > 2. ) CH = SIGN( 2._ReKi, CH )
...
IF ( SWIRL ) THEN
   IF ( EquilDT )  THEN     ! USE PROP-PC style tangential induction equation with the addition of the drag term.
      ...
      ELSEIF ( ABS( SPhi ) > 0.01 )  THEN   ! Tangential velocity near zero, phi near 90 degrees.
         A2P = SOLFACT*( CLA*SPhi - CDA*SIGN( 0.01_ReKi, CPhi ) )*( 1.0 + A2P )*VNROTOR2/( 4.0*LOSS*SPhi*SIGN( 0.01_ReKi, CPhi ) )
      ELSE   ! Normal velocity near zero, phi near 0 degrees.
         A2P = SOLFACT*( CLA*SIGN( 0.01_ReKi, SPhi ) - CDA*CPhi )*( 1.0 + A2P )*VNROTOR2/( 4.0*LOSS*SIGN( 0.01_ReKi, SPhi )*CPhi )
      ENDIF
   ELSE
...
!====================================================================================================
   ...
   ELSE                                                     ! There are multiple Re rows
      ...
      TwrElRe = GetReynolds( VelHor, 2.0_ReKi*TwrElRad )
...
ELSE
   ...
   FSP(J,IBLADE) = TEMP * TEMP * SIGN ( 1._ReKi, TEMP )
   ...
   ELSE
      ...
      FSPC(J,IBLADE) = TEMP * TEMP * SIGN ( 1._ReKi, TEMP )
...
SRFP = SQRT( ABS( FSP(J,IBLADE) ) ) * SIGN( 1._ReKi, FSP(J,IBLADE) ) + 1.
FK   = 0.25 * SRFP * SRFP
...
CNIQ = MIN( ABS( CNI+CNQ ), 1. ) * SIGN( 1._ReKi, CNI+CNQ )
...
SRFP = SQRT( ABS(FP) )  * SIGN( 1._ReKi, FP ) + 1.
SRFPC= SQRT( ABS(FPC) ) * SIGN( 1._ReKi,FPC )

...
IF ( CompHydro .AND. SaveGrphcs .AND. ( WaveMod /= 4 ) )  THEN ! .TRUE. if we are using the undocumented monopile or platform features .AND. SaveGrphcs is enabled, but not with GH Bladed wave data
   ...
   ! Free surface elevation JOINT and MOTION:
      ...
      WRITE (UnAD,FmtText     )  ', FUNCTION = '//TRIM(Flt2LStr( 0.0009868617_ReKi ))//'*VARVAL('//TRIM(Int2LStr( TmpID2 ))//')'// &  ! NOTE: 0.0009868617kN/m     = 0.001*  Mass*NaturalFrequency^2            = 0.001*  (9.999E-4kg)*(5Hz)^2     = 0.001*  (9.999E-4kg)*(31.41593rad/s)^2
                                           ' - '//TRIM(Flt2LStr( 0.0000439779_ReKi ))//'*VZ('//TRIM(Int2LStr( TmpID ))//',0,10,0)'    ! NOTE: 0.0000439779kN/(m/s) = 0.001*2*Mass*NaturalFrequency*DampingRatio = 0.001*2*(9.999E-4kg)*(5Hz)*(0.7) = 0.001*2*(9.999E-4kg)*(31.41593rad/s)*(0.7)
...
IF ( ( PtfmModel == 3 ) .AND. CompHydro .AND. SaveGrphcs )  THEN  ! .TRUE. if we have floating offshore turbine and we are using the undocumented platform features .AND. SaveGrphcs is enabled.
   ...
   ! Nodal position control:
            ...
            WRITE (UnAD,FmtText            )  ', FX = '//TRIM(Flt2LStr( 0.0009868617_ReKi ))// &  ! NOTE: 0.0009868617kN/m     = 0.001*  Mass*NaturalFrequency^2            = 0.001*  (9.999E-4kg)*(5Hz)^2     = 0.001*  (9.999E-4kg)*(31.41593rad/s)^2
                                                      '*VARVAL('//TRIM(Int2LStr( TmpID + 4000 ))//')'// &
                                                  ' - '//TRIM(Flt2LStr( 0.0000439779_ReKi ))// &  ! NOTE: 0.0000439779kN/(m/s) = 0.001*2*Mass*NaturalFrequency*DampingRatio = 0.001*2*(9.999E-4kg)*(5Hz)*(0.7) = 0.001*2*(9.999E-4kg)*(31.41593rad/s)*(0.7)
                                                      '*VX('//TRIM(Int2LStr( TmpID ))//',0,10,0)\'
            WRITE (UnAD,FmtText            )  ', FY = '//TRIM(Flt2LStr( 0.0009868617_ReKi ))// &  ! NOTE: 0.0009868617kN/m     = 0.001*  Mass*NaturalFrequency^2            = 0.001*  (9.999E-4kg)*(5Hz)^2     = 0.001*  (9.999E-4kg)*(31.41593rad/s)^2
                                                      '*VARVAL('//TRIM(Int2LStr( TmpID + 5000 ))//')'// &
                                                  ' - '//TRIM(Flt2LStr( 0.0000439779_ReKi ))// &  ! NOTE: 0.0000439779kN/(m/s) = 0.001*2*Mass*NaturalFrequency*DampingRatio = 0.001*2*(9.999E-4kg)*(5Hz)*(0.7) = 0.001*2*(9.999E-4kg)*(31.41593rad/s)*(0.7)
                                                      '*VY('//TRIM(Int2LStr( TmpID ))//',0,10,0)\'
            WRITE (UnAD,FmtText            )  ', FZ = '//TRIM(Flt2LStr( 0.0009868617_ReKi ))// &  ! NOTE: 0.0009868617kN/m     = 0.001*  Mass*NaturalFrequency^2            = 0.001*  (9.999E-4kg)*(5Hz)^2     = 0.001*  (9.999E-4kg)*(31.41593rad/s)^2
...
DO K = 1,NumBl       ! Loop through all blades
   ...
   !   oriented with the reference axis for precurved and preswept blades:
      ...
      CALL SmllRotTrans( 'blade prebend', Slopexb, Slopeyb, 0.0_ReKi, TransMat, TRIM(Num2LStr(ZTime))//' s' )   ! Get the transformation matrix, TransMat, from the pitch axis to the reference axis coordinate system.
...
WRITE (UnAD,FmtText  )  ', I = 2010'
WRITE (UnAD,FmtText  )  ', J = 1010'
WRITE (UnAD,FmtText  )  ', FUNCTION = -'//TRIM(Flt2LStr( YawSpr *0.001_ReKi ))//'*VARVAL(2013) - '// &
                                          TRIM(Flt2LStr( YawDamp*0.001_ReKi ))//'*VARVAL(2014)'
...
CASE ( 1 )              ! Standard (using inputs from the FAST furling input file).
   ...
   WRITE (UnAD,FmtText  )  ', -'//TRIM(Flt2LStr( TFrlUSSpr*0.001_ReKi ))//'*( AZ(5040,2040) - '//TRIM(Flt2LStr( TFrlUSSP ))//' ) )'
   ...
   WRITE (UnAD,FmtText  )  ', -'//TRIM(Flt2LStr( TFrlDSSpr*0.001_ReKi ))// &
   ...
   WRITE (UnAD,FmtText  )  ', FUNCTION = IF( AZ(5040,2040) - '//TRIM(Flt2LStr( TFrlUSDP ))// &
                           ': 0, 0, -'//TRIM(Flt2LStr( TFrlUSDmp*0.001_ReKi ))//'*WZ(5040,2040,2040) )'
   ...
   WRITE (UnAD,FmtText  )  ', FUNCTION = IF( AZ(5040,2040) - '//TRIM(Flt2LStr( TFrlDSDP ))// &
                           ': -'//TRIM(Flt2LStr( TFrlDSDmp*0.001_ReKi ))//'*WZ(5040,2040,2040), 0, 0 )'
   ...
   WRITE (UnAD,FmtText  )  ', I = 5040'
   WRITE (UnAD,FmtText  )  ', J = 2040'
   WRITE (UnAD,FmtText  )  ', FUNCTION = IF( WZ(5040,2040,2040): '//TRIM(Flt2LStr( TFrlCDmp*0.001_ReKi ))// &
                           ', 0, -'//TRIM(Flt2LStr( TFrlCDmp*0.001_ReKi ))//' )'
...
CASE ( 1 )              ! Standard (using inputs from the FAST furling input file).
   ...
   WRITE (UnAD,FmtText  )  ', -'//TRIM(Flt2LStr( RFrlUSSpr*0.001_ReKi ))//'*( AZ(2130,2030) - '//TRIM(Flt2LStr( RFrlUSSP ))//' ) )'
   ...
   WRITE (UnAD,FmtText  )  ', -'//TRIM(Flt2LStr( RFrlDSSpr*0.001_ReKi ))// &
   ...
   WRITE (UnAD,FmtText  )  ', FUNCTION = IF( AZ(2130,2030) - '//TRIM(Flt2LStr( RFrlUSDP ))// &
                           ': 0, 0, -'//TRIM(Flt2LStr( RFrlUSDmp*0.001_ReKi ))//'*WZ(2130,2030,2030) )'
   ...
   WRITE (UnAD,FmtText  )  ', FUNCTION = IF( AZ(2130,2030) - '//TRIM(Flt2LStr( RFrlDSDP ))// &
                           ': -'//TRIM(Flt2LStr( RFrlDSDmp*0.001_ReKi ))//'*WZ(2130,2030,2030), 0, 0 )'
   ...
   WRITE (UnAD,FmtText  )  ', I = 2130'
   WRITE (UnAD,FmtText  )  ', J = 2030'
   WRITE (UnAD,FmtText  )  ', FUNCTION = IF( WZ(2130,2030,2030): '//TRIM(Flt2LStr( RFrlCDmp*0.001_ReKi ))// &
                           ', 0, -'//TRIM(Flt2LStr( RFrlCDmp*0.001_ReKi ))//' )'
...
CASE ( 1 )              ! Standard (using inputs from the primary FAST input file).
   ...
   WRITE (UnAD,FmtText  )  ', -SIGN( '//TRIM(Flt2LStr( TeetSSSp*0.001_ReKi ))//'*( ABS(AZ(4010,3310)) - '// &
   ...
   WRITE (UnAD,FmtText  )  ', -SIGN( '//TRIM(Flt2LStr( TeetHSSp*0.001_ReKi ))//'*( ABS(AZ(4010,3310)) - '// &
   ...
   WRITE (UnAD,FmtText  )  ', FUNCTION = IF( ABS(AZ(4010,3310)) - '//TRIM(Flt2LStr( TeetDmpP ))//': 0, 0, -'// &
                           TRIM(Flt2LStr( TeetDmp*0.001_ReKi ))//'*WZ(4010,3310,3310) )'
   ...
   WRITE (UnAD,FmtText  )  ', I = 4010'
   WRITE (UnAD,FmtText  )  ', J = 3310'
   WRITE (UnAD,FmtText  )  ', FUNCTION = IF( WZ(4010,3310,3310): '//TRIM(Flt2LStr( TeetCDmp*0.001_ReKi ))//', 0, -'// &
                           TRIM(Flt2LStr( TeetCDmp*0.001_ReKi ))//' )'
...
!                            ', '//TRIM(Int2LStr( NumBl       ))//','
   ...
   WRITE (UnAD,FmtText   )  ', I = '   //TRIM(Int2LStr( TmpID     ))
   WRITE (UnAD,FmtText   )  ', J = '   //TRIM(Int2LStr( TmpID2    ))
   WRITE (UnAD,FmtText   )  ', FUNCTION = -'//TRIM(Flt2LStr( BPActrSpr*0.001_ReKi ))//                &
                            '*VARVAL('//TRIM(Int2LStr( TmpID + 1 ))//') - '//                    &
                            TRIM(Flt2LStr( BPActrDmp*0.001_ReKi ))//'*WZ('//TRIM(Int2LStr( TmpID ))// &
...
CALL MakeADM_WrICArraysR( (/ REAL(NumBl,ReKi),     REAL(BldNodes,ReKi),   REAL(TwrNodes,ReKi),   REAL(NBlGages,ReKi),  & 
                             ...
                             TipRad,               TStart               , REAL(PtfmLdMod,4),       REAL(TwrLdMod,4),       ProjArea,  &

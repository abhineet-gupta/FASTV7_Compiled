...
DO K = 1,NumBl
   DO I = 1,NBlGages
      ...
      !   length is 1/2 of DRNodes().
         ...
         CALL CrossProd( TmpVec, ( 0.25_ReKi*DRNodes(BldGagNd(I)) )*j3(K,:), TmpVec2 )                              ! Portion of MomMGagB associated with 1/2 of the strain gage element
...
DO I=1,NTwGages
   ...
   CALL CrossProd( TmpVec, ( 0.25_ReKi*DHNodes( TwrGagNd(I)) )*a2, TmpVec2 )              ! Portion of MomFGagT associated with 1/2 of the strain gage element
...
DO K = 1,NumBl          ! Loop through the blades
   ...
   Shape  = SHP( 1.0_ReKi, BldFlexL, BldFl1Sh(:,K), 2 )
   ...
   Shape  = SHP( 1.0_ReKi, BldFlexL, BldFl2Sh(:,K), 2 )
   ...
   Shape  = SHP( 1.0_ReKi, BldFlexL, BldEdgSh(:,K), 2 )
...
TwrFASF(1,TTopNode,2) = SHP( 1.0_ReKi, TwrFlexL, TwFAM1Sh(:), 2 )
TwrFASF(2,TTopNode,2) = SHP( 1.0_ReKi, TwrFlexL, TwFAM2Sh(:), 2 )
TwrFASF(1,TTopNode,1) = SHP( 1.0_ReKi, TwrFlexL, TwFAM1Sh(:), 1 )
TwrFASF(2,TTopNode,1) = SHP( 1.0_ReKi, TwrFlexL, TwFAM2Sh(:), 1 )
TwrFASF(1,TTopNode,0) = SHP( 1.0_ReKi, TwrFlexL, TwFAM1Sh(:), 0 )
TwrFASF(2,TTopNode,0) = SHP( 1.0_ReKi, TwrFlexL, TwFAM2Sh(:), 0 )

TwrSSSF(1,TTopNode,2) = SHP( 1.0_ReKi, TwrFlexL, TwSSM1Sh(:), 2 )
TwrSSSF(2,TTopNode,2) = SHP( 1.0_ReKi, TwrFlexL, TwSSM2Sh(:), 2 )
TwrSSSF(1,TTopNode,1) = SHP( 1.0_ReKi, TwrFlexL, TwSSM1Sh(:), 1 )
TwrSSSF(2,TTopNode,1) = SHP( 1.0_ReKi, TwrFlexL, TwSSM2Sh(:), 1 )
TwrSSSF(1,TTopNode,0) = SHP( 1.0_ReKi, TwrFlexL, TwSSM1Sh(:), 0 )
TwrSSSF(2,TTopNode,0) = SHP( 1.0_ReKi, TwrFlexL, TwSSM2Sh(:), 0 )
...
ADInterfaceComponents%Hub%Position(:)       = (/ rP(1), -1._ReKi*rP(3), rP(2) - PtfmRef /)


   ! Rotor furl position should be rP instead of rV, but AeroDyn needs this for the
   ! HubVDue2Yaw calculation:
   
ADInterfaceComponents%RotorFurl%Position(:) = (/ rV(1), -1._ReKi*rV(3), rV(2) - PtfmRef /)
   
ADInterfaceComponents%Nacelle%Position(:)   = (/ rO(1), -1._ReKi*rO(3), rO(2) - PtfmRef /)
   
   ! Tower base position should be rT(0) instead of rZ, but AeroDyn needs this for
   ! the HubVDue2Yaw calculation:
ADInterfaceComponents%Tower%Position(:)     = (/ rZ(1), -1._ReKi*rZ(3), rZ(2) - PtfmRef /)
...
ADInterfaceComponents%RotorFurl%Orientation(:,1) = (/      c1(1), -1._ReKi*c3(1),     c2(1) /) 
ADInterfaceComponents%RotorFurl%Orientation(:,2) = (/ -1._ReKi* c1(3),     c3(3), -1._ReKi*c2(3) /)
ADInterfaceComponents%RotorFurl%Orientation(:,3) = (/      c1(2), -1._ReKi*c3(2),     c2(2) /)

      ! Nacelle orientation (note the different order than hub and blade root!)

ADInterfaceComponents%Nacelle%Orientation(:,1) = (/      d1(1), -1._ReKi*d3(1),     d2(1) /) 
ADInterfaceComponents%Nacelle%Orientation(:,2) = (/ -1._ReKi* d1(3),     d3(3), -1._ReKi*d2(3) /)
ADInterfaceComponents%Nacelle%Orientation(:,3) = (/      d1(2), -1._ReKi*d3(2),     d2(2) /)
...
ADInterfaceComponents%Hub%RotationVel(:)       = (/ AngVelEL(1), -1._ReKi*AngVelEL(3), AngVelEL(2) /) 
ADInterfaceComponents%RotorFurl%RotationVel(:) = (/ AngVelER(1), -1._ReKi*AngVelER(3), AngVelER(2) /) 
ADInterfaceComponents%Nacelle%RotationVel(:)   = (/ AngVelEN(1), -1._ReKi*AngVelEN(3), AngVelEN(2) /)
ADInterfaceComponents%Tower%RotationVel(:)     = (/ AngVelEX(1), -1._ReKi*AngVelEX(3), AngVelEX(2) /)
...
!JASON: USE TipNode HERE INSTEAD OF BldNodes IF YOU ALLOCATE AND DEFINE n1, n2, n3, m1, m2, AND m3 TO USE TipNode.  THIS WILL REQUIRE THAT THE AERODYNAMIC AND STRUCTURAL TWISTS, AeroTwst() AND ThetaS(), BE KNOWN AT THE TIP!!!
   ...
   !   in the inertia frame associated with everything but the QD2T()'s:
      ...
      ADAeroMarkers%Blade(J,K)%TranslationVel(:)= (/ LinVelES(1), -1._ReKi*LinVelES(3),  LinVelES(2)  /)  !AeroDyn's coordinates
...
SgnPrvLSTQ = NINT( SIGN( 1.0_ReKi, DOT_PRODUCT( MomLPRot, e1 ) ) )
...
DO J = 1,TwrNodes ! Loop through the tower nodes / elements
   ...
   CALL SmllRotTrans( 'tower deflection', ThetaSS, 0.0_ReKi, ThetaFA, TransMat, TRIM(Num2LStr(ZTime))//' s' )   ! Get the transformation matrix, TransMat, from tower-base to tower element-fixed coordinate systems.
...
CALL SmllRotTrans( 'tower deflection', ThetaSS, 0.0_ReKi, ThetaFA, TransMat, TRIM(Num2LStr(ZTime))//' s' )   ! Get the transformation matrix, TransMat, from tower-base to tower-top/base-plate coordinate systems.
...
!JASON: USE TipNode HERE INSTEAD OF BldNodes IF YOU ALLOCATE AND DEFINE n1, n2, n3, m1, m2, AND m3 TO USE TipNode.  THIS WILL REQUIRE THAT THE AERODYNAMIC AND STRUCTURAL TWISTS, AeroTwst() AND ThetaS(), BE KNOWN AT THE TIP!!!
   ...
   ! Blade element-fixed coordinate system aligned with local structural axes:
      ...
      CALL SmllRotTrans( 'blade deflection', ThetaLxb, ThetaLyb, 0.0_ReKi, TransMat, TRIM(Num2LStr(ZTime))//' s' ) ! Get the transformation matrix, TransMat, from blade coordinate system aligned with local structural axes (not element fixed) to blade element-fixed coordinate system aligned with local structural axes.
